...
| 고도 기준 | **기압계 AGL** (발사대 0점, 로켓·페이로드 공유 datum) — §3.2 결정 2026-07-21 | GPS 수직오차 회피 |
...
### 3.2 결정 완료 — 고도 기준: **기압계 AGL** (2026-07-21)
초안(v0.1)은 GPS `hMSL 통일`을 권장했으나, **기압계 사용으로 결정**했다.

- **이유:** MAX-M10S GPS 수직오차는 수 m~십수 m로 수평보다 크다. 알고리즘이 쓰는 건 상대 고도차(dD) 하나뿐인데, 기압계의 **상대 고도차**는 수십 cm급으로 훨씬 정밀하다. 이득이 가장 큰 구간은 **근거리·고각**(발사 직후 머리 위 근처)으로, 추적에서 제일 어려운 구간과 일치한다.
- **기준 통일:** 로켓·페이로드 **둘 다** 기압계를 달고, 발사 직전 **같은 발사대에서 0점(AGL)** 을 잡는다. 페이로드가 로켓에 실려 같은 발사대에서 출발하므로 두 유닛이 같은 datum을 공유 → `dD = 페이로드AGL − 로켓AGL`이 그대로 물리 상대고도와 일치.
- **역할 분담:** **수평(위경도·속도)=GPS**, **수직(고도)=기압계**. GPS는 버리지 않는다.
- 상세 설계는 §3.5. GPS `hMSL`(offset 36)은 **기압계 고장·부재 시 폴백**으로만 남긴다.

### 3.5 기압계 고도 (신규 — §3.2 결정 반영)
- **센서:** BMP390 / MS5611 급 (I2C). BNO085도 I2C이므로 **주소 충돌만 확인**(BMP390 0x77/0x76). 로켓·페이로드 각 1개.
- **변환식(ISA 대류권):** `h = 44330.77·(1 − (P/P0)^0.190263)`. `P0` = 발사대 0점에서 측정한 기준기압. 코드: **`baro.h`의 `baroAltitudeAgl()`**.
- **0점:** `baro.h`의 `BaroAgl` 클래스. 부팅 안정화 후 지상에서 `zeroAverage(samples)`로 다중 샘플 평균 0점. 이후 `altitude(P)`가 AGL 반환.
- **주입 경로(코어):** 페이로드 고도는 `CoarseTracker::onBaro(aglM, nowMs)`로 공급. `compute()`는 기압계가 신선하면(`BARO_TIMEOUT_S` 0.5s) 기압계 AGL, 아니면 GPS `hMSL` 폴백. 로켓 고도는 패킷 `alt_mm`(=로켓 기압계 AGL, §5.1).
- **주의(비행 중):** 로켓 고속(부스트) 구간은 동압으로 정압 포트가 왜곡된다 → 벤트 베이 권장. 코스트/하강 구간이 신뢰도 높음. 부스트 구간 GPS 폴백/sanity-check 블렌딩은 **v2 후보**(현재 미구현).
- **검증:** `test_cpp/test_baro.cpp`(변환식·0점·두 유닛 상대고도, 19개) + `test_coarse.cpp` §7(주입·폴백). 전부 부품 없이 PC에서 통과.
...
struct GpsFix {
  ...
  float    alt_m;         // hMSL/1000 — 기압계 부재/고장 시 폴백 (§3.2 결정: 기본은 기압계 AGL)
  float    vN, vE, vD;    // m/s
...
struct RocketPacket {          // little-endian, 고정 크기
  ...
  int32_t  alt_mm;             // 기압계 AGL, mm (발사대 0점 — §3.2 결정 2026-07-21. 와이어 포맷 불변)
...
2. **패킷 포맷 합의**: §5.1 구조를 로켓 팀과 확정(필드·엔디안·CRC 다항식). **※ `alt_mm`은 기압계 AGL로 채운다(§3.2)** — 로켓팀 인터페이스 항목.
3. ~~고도 기준 통일~~ → **결정됨: 기압계 AGL(발사대 0점)** (§3.2, 2026-07-21). 남은 실무: **로켓측 기압계 장착·0점 절차** 로켓팀 합의.
4. **GPS 물리 연결**: UART(남는 포트) vs I2C(BNO085와 버스 공유). `Serial1`은 DXL 점유 중. **+ 기압계도 I2C** → BNO085/기압계/GPS(I2C 시) 주소 충돌 점검.
